...
import MethodologyPanel from '../components/Multiples/MethodologyPanel'
...
import { BarChart3, LineChart, Globe2, BookOpen } from 'lucide-react'
import clsx from 'clsx'
import {
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell, Legend,
} from 'recharts'
import { fetchSectorIndices, MOEX_SECTOR_INDICES } from '../lib/moex'
...
function GlobalBenchmarkChart({ sectorId }: { sectorId: string }) {
  ...
  if (!data) return (
    <div className="flex items-center justify-center h-32 text-text-muted text-sm">
      Глобальные бенчмарки для этого сектора пока не загружены
    </div>
  )
    ...
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={data} margin={{ top: 0, right: 16, left: 0, bottom: 0 }}>
        ...
        <Bar dataKey="evEbitda" name="EV/EBITDA" radius={[3, 3, 0, 0]}>
          {data.map((entry, i) => <Cell key={i} fill={entry.region === 'Россия' ? '#3b82f6' : '#64748b'} />)}
        </Bar>
...
export default function MultiplesPage() {
  ...
  // Live MOEX data — auto-refresh каждую минуту
  const live = useLiveData(useCallback(() => fetchSectorIndices(), []), { intervalMs: 60_000 })
  ...
  const moexCfg = selected ? MOEX_SECTOR_INDICES[selected] : undefined
  const liveQuote = moexCfg ? live.data?.get(moexCfg.ticker) ?? null : null
  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 p-6 space-y-6 overflow-y-auto">
        {/* Live bar */}
        <div className="flex items-center justify-between gap-3 bg-surface-2 border border-border-subtle rounded-xl px-4 py-3">
          <div className="flex items-center gap-2 text-sm text-text-secondary">
            <BookOpen size={14} className="text-accent" />
            Данные мультипликаторов рассчитываются на основе индексов MOEX и отчётности МСФО эмитентов
          </div>
          <LiveIndicator
                  ...
                  <button
                    ...
                    className={clsx(
                      ...
                      showMethodology
                        ...
                        : 'bg-surface-3 text-text-secondary border-border-subtle hover:border-accent/40'
                    )}
                  >
                    <BookOpen size={12} /> Методология
                  </button>
                  <button
              ...
              <div className="p-6">
                ...
                {tab === 'chart' && (
                  <div className="text-text-secondary text-sm leading-relaxed">
                    ...
                    <p className="mt-2">
                      Общая рыночная капитализация сектора —&nbsp;
                      <strong className="text-text-primary">
                        ₽{selectedSector.marketCapBn >= 1000
                          ? `${(selectedSector.marketCapBn / 1000).toFixed(1)} трлн`
                          : `${selectedSector.marketCapBn} млрд`}
                      </strong>, в выборке&nbsp;
                      <strong className="text-text-primary">{selectedSector.companiesCount}</strong>&nbsp;компаний.
                    </p>
                  </div>
                )}

                {tab === 'history' && history && <HistoryLineChart data={history} />}
                {tab === 'history' && !history && (
                  <div className="flex items-center justify-center h-32 text-text-muted text-sm">
                ...
                {tab === 'global' && <GlobalBenchmarkChart sectorId={selected!} />}
              </div>
            </div>
